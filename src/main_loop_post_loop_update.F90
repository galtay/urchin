...
subroutine main_loop()    
  ...
  do ipar = 1, size(psys%par)
     psys%par(ipar)%xHI_tmp = psys%par(ipar)%xHI
!     psys%par(ipar)%xHI_tmp = zero
  end do
     ...
     over_pars: do list_indx = 1, size(psys%par)
        ...
        if (flag_cycle) then
           ...
           bck_par = initialize_bckgnd_particle( psys%par(psys_indx), &
                GHI=GHI_int_thin, caseA=.true.)

           err = solve_bckgnd_particle_xH_eq(bck_par)
           psys%par(psys_indx)%xHI_tmp = bck_par%xHI
...
!$OMP do
        over_pixels: do ipix = 0, npix-1
           ...
           call calc_tau_for_healpixel( &
                psys_indx, ipix, sky_frac, nside, minispec, &
                raylist_pix(ipix), tauHI_ray_pix(ipix), tauHI_nbr_pix(ipix), &
                tauHI_far_pix(ipix), tauH_nbr_pix(ipix) )
...
!$OMP end parallel
        ...
        if (tauHI_eff >= CV%ShldTauThresh) then 
           ...
           bck_par = initialize_bckgnd_particle( &
                psys%par(psys_indx), GHI_total, caseA=.false.)
        else
           bck_par = initialize_bckgnd_particle( &
                psys%par(psys_indx), GHI_total, caseA=.true.)
        endif
...
!        if (ndone == 100000) stop 'profiling'
     ...
     ! after first particle loop, update all
     !------------------------------------------  
     do ipar = 1, size(psys%par)

        xHInow = psys%par(ipar)%xHI_tmp
        xHIold = psys%par(ipar)%xHI

        if ( xHInow - xHIold < CV%ConvTol * xHIold ) then
           n_converged = n_converged + 1
        endif

        psys%par(ipar)%xHI = psys%par(ipar)%xHI_tmp
...
!-----------------------------------------------------------
subroutine calc_tau_for_healpixel(psys_indx, ipix, sky_frac, nside, minispec, &
     raylist, tauHI_ray, tauHI_nbr, tauHI_far, tauH_nbr )

  integer(i8b), intent(in) :: psys_indx          !< index of active particle in psys
  integer(i4b), intent(in) :: ipix               !< pixel ID
  real(r8b), intent(in) :: sky_frac              !< fraction of sky for this pixel
  integer(i4b), intent(in) :: nside              !< Healpix Nside
  type(mini_spec_type), intent(in) :: minispec   !< spectrum object
...
!  real(r8b), intent(out) :: GHI_ray              !< GHI from tauHI_ray
!  real(r8b), intent(out) :: GHI_far              !< GHI from tauHI_far
 
  real(r8b) :: start(3)                          !< start position of ray
  real(r8b) :: vec(3)                            !< unit direction of ray

  integer :: ihit                                !< intersection counter
  integer :: hit_indx                            !< index of hit particle in psys
...
subroutine calc_ray_stats(psys_indx, ipix, raylist, nside)
  ...
  character(clen) :: rayfile
  integer(i4b) :: raystatlun
  integer(i4b) :: ihit
  integer(i4b) :: hit_indx
